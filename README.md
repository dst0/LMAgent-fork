...
```bash
pip install Pillow              # image uploads in the web UI
...
- **Todo & Plan tools** — bookkeeping helpers; the runtime now owns their statuses while the agent drafts and edits work items
...
| Shell | `shell` — sandboxed, up to 120s timeout, up to 2 GB memory |
| Git | `git_status`, `git_diff`, `git_add`, `git_commit`, `git_branch` |
| Todos | `todo_add`, `todo_list` |
| Planning | `task_state_get`, `task_reconcile` |
...
| `THINKING_MODEL` | `false` | Strip `<think>` blocks (set `true` for QwQ, DeepSeek-R1) |
...
| Variable | Default | Description |
|---|---|---|
| `SUMMARIZATION_THRESHOLD` | `40000` | Token count that triggers context compaction |
| `KEEP_RECENT_MESSAGES` | `30` | Messages to retain after compaction |
| `ENABLE_SUMMARIZATION` | `true` | Enable automatic context compaction |
| `MAX_SUB_AGENT_ITERATIONS` | `25` | Iteration cap for sub-agents |

### Limits & Tuning

| Variable | Default | Description |
|---|---|---|
| `LLM_MAX_TOKENS` | `-1` | Max output tokens (`-1` = unlimited) |
| `LLM_MAX_RETRIES` | `3` | API retry attempts |
| `LLM_RETRY_DELAY` | `3.0` | Seconds between retries |
| `MAX_SAME_TOOL_STREAK` | `8` | Consecutive same-tool calls before loop warning |
| `MAX_NO_PROGRESS_ITERS` | `15` | Iterations without progress before warning |
| `MAX_ERRORS` | `5` | Consecutive errors before stopping |
| `MAX_TOOL_OUTPUT` | `500000` | Tool output truncation limit (bytes) |
| `MAX_FILE_READ` | `1000000` | Max file read size (bytes) |
| `MAX_GREP_RESULTS` | `50` | Maximum grep matches returned |
| `MAX_LS_ENTRIES` | `100` | Maximum directory entries returned by `ls` |

### Feature Flags

| Variable | Default | Description |
|---|---|---|
| `ENABLE_MCP` | `true` | Load MCP tool servers from `.lmagent/mcp.json` |
| `ENABLE_TODO_TRACKING` | `true` | Enable todo list tracking |
| `ENABLE_PLAN_ENFORCEMENT` | `true` | Enforce execution planning |
| `AUTO_SAVE_SESSION` | `true` | Auto-save conversation state to disk |

### Scheduler

| Variable | Default | Description |
|---|---|---|
| `SCHEDULER_POLL_INTERVAL` | `60` | Seconds between scheduler inbox checks |

### Sub-Agent Tuning

| Variable | Default | Description |
|---|---|---|
| `BRIEF_DATA_BUDGET` | `8000` | Token budget for extracted data in a sub-agent brief |
| `ARTIFACT_SCAN_WINDOW` | `600` | Seconds to scan for misplaced artifact files after a sub-agent completes |

### Web Server

| Variable | Default | Description |
|---|---|---|
| `AGENT_PORT` | `7860` | HTTP port for `agent_web.py` |
| `AGENT_HOST` | `0.0.0.0` | Bind address |
| `AGENT_TOKEN` | *(random 6-digit PIN)* | Fixed auth PIN; generated on startup if unset |
| `AGENT_CERT` | *(blank)* | Path to SSL certificate for HTTPS |
| `AGENT_KEY` | *(blank)* | Path to SSL key for HTTPS |

### Sandbox

| Variable | Default | Description |
|---|---|---|
| `FORCE_DOCKER` | `false` | Use Docker on macOS/Linux instead of process-group fallback |
| `DOCKER_IMAGE` | `python:3.12-slim` | Docker image for the sandbox container |
| `DOCKER_NETWORK` | `bridge` | Docker network mode (`bridge` = internet, `none` = air-gapped) |

### Messaging (optional — only set what you use)
...
| `TWILIO_FROM` | Twilio sender phone number, e.g. `+12025551234` |
...
python agent_main.py --workspace /path/to/project "fix the bug"

# Disable optional features
python agent_main.py --no-mcp --no-sub-agents "simple task"

# Set permission mode
python agent_main.py --permission auto "batch rename these files"
```
...
**Thinking models (QwQ, DeepSeek-R1):** Set `THINKING_MODEL=true`. Reasoning tokens are stripped before being fed back into context so they don't crowd out actual task state.
...
**Large batch jobs:** The runtime checkpoints task progress automatically so it survives context compaction and restarts.
...
`agent_web.py` exposes the following HTTP API. All routes except `/` and the QR sign-in page require the PIN token via `X-Token` header, `?token=` query parameter, or `agent_token` cookie.

| Method | Route | Description |
|---|---|---|
| `GET` | `/` | Serve the web UI |
| `POST` | `/chat` | Submit a task message |
| `GET` | `/stream` | SSE stream of agent events (tokens, tool calls, status) |
| `POST` | `/stop` | Stop a running agent |
| `POST` | `/new` | Start a fresh session |
| `GET` | `/status` | Agent state, session ID, workspace, permission mode |
| `GET` | `/sessions` | List recent sessions |
| `GET` | `/tools` | List available tools for the current session |
| `POST` | `/command` | Execute a slash command (e.g. `{"command": "/todo"}`) |
| `POST` | `/upload` | Upload an image file (jpg/png/gif/webp) |
| `GET` | `/filetree` | Directory listing for the workspace file browser |
| `GET` | `/fileread` | Read a workspace file (up to 100 KB preview) |
| `GET` | `/serve/<path>` | Serve workspace files with asset URL rewriting |
| `GET` | `/workspace/mtime` | Latest file modification time in the workspace |
| `GET` | `/session/todos` | Load todos for a session |
| `GET` | `/session/tasks` | Load task state for a session |
| `GET` | `/session/plan` | Load execution plan for a session |
| `GET` | `/session/history` | Load conversation history for a session |
| `GET` | `/session/eventlog` | Load SSE event log for a session |
| `POST` | `/sms` | Twilio webhook for inbound SMS |
| `GET` | `/messaging/status` | Status of all messaging platform connections |
| `POST` | `/messaging/mode` | Switch the active messaging platform |
| `GET` | `/messaging/feed` | Recent message feed for the web UI messaging panel |

---

## Debugging

Session data is written to `.lmagent/` inside your workspace:

| Path | Contents |
|---|---|
| `.lmagent/sessions/<session_id>/data.json` | Full conversation history |
| `.lmagent/todos/<session_id>/todos.json` | Todo list |
| `.lmagent/plans/<session_id>/plan.json` | Execution plan |
| `.lmagent/state/<session_id>/task_state.json` | Task checkpoint (batch jobs) |
| `.lmagent/agents/<agent_id>/brief.json` | Sub-agent input brief |
| `.lmagent/agents/<agent_id>/result.json` | Sub-agent completion result |
| `.lmagent/agents/<agent_id>/scope/` | Sub-agent scratch directory |
| `.lmagent/mcp.json` | MCP server configuration |
| `.lmagent/messaging_sessions.json` | Platform → session ID mappings |
| `.lmagent/.soul.md` | Agent personality config |

**Inspecting sub-agent briefs:** To keep `brief.json` and `result.json` after a run, remove the `cleanup_session_dirs(workspace)` call at the end of `run_agent()` in `agent_main.py`.
